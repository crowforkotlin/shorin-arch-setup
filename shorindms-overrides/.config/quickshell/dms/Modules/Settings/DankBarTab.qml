...
Item {
    ...
    function createNewBar() {
        ...
        const newBar = {
            ...
            edgeInset: defaultBar.edgeInset ?? defaultBar.spacing ?? 4,
    ...
    DankFlickable {
        ...
        Column {
            ...
            SettingsCard {
                ...
                settingKey: "barCenteringMode"
                visible: selectedBarConfig?.enabled ?? false

                SettingsToggleRow {
                    text: I18n.tr("Use Waybar-style Centering")
                    checked: SettingsData.centeringMode === "geometric"
                    onToggled: toggled => SettingsData.set("centeringMode", toggled ? "geometric" : "index")
                }
            }
            ...
            SettingsCard {
                ...
                SettingsSliderRow {
                    id: outerEdgeInsetSlider
                    text: I18n.tr("Outer Edge Inset")
                    value: selectedBarConfig?.edgeInset ?? selectedBarConfig?.spacing ?? 4
                    minimum: 0
                    maximum: 32
                    defaultValue: 4
                    onSliderDragFinished: finalValue => {
                        SettingsData.updateBarConfig(selectedBarId, {
                            edgeInset: finalValue
                        });
                    }

                    Binding {
                        target: outerEdgeInsetSlider
                        property: "value"
                        value: selectedBarConfig?.edgeInset ?? selectedBarConfig?.spacing ?? 4
                        restoreMode: Binding.RestoreBinding
                    }
