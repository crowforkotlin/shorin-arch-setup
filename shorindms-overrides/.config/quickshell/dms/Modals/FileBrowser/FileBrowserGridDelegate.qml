...
StyledRect {
    ...
    property bool previewCapable: (!delegateRoot.fileIsDir && (isImage || isVideo)) || (weMode && delegateRoot.fileIsDir)
    property bool previewReady: previewCapable && gridPreviewImage.status === Image.Ready
    property bool previewLoading: {
        if (!previewCapable)
            return false;
        if (!delegateRoot.fileIsDir && isVideo)
            return _videoThumbPending;
        if (gridPreviewImage.imagePath === "")
            return false;
        return gridPreviewImage.status === Image.Loading || gridPreviewImage.status === Image.Null;
    }
    ...
    Column {
        ...
        Item {
            ...
            MultiEffect {
                ...
                visible: delegateRoot.previewCapable
                opacity: delegateRoot.previewReady ? 1 : 0
            ...
            DankNFIcon {
                ...
                visible: !delegateRoot.previewCapable || opacity > 0.01
                opacity: delegateRoot.previewCapable && delegateRoot.previewReady ? 0 : 1
            ...
            BusyIndicator {
                anchors.centerIn: parent
                width: Math.max(22, iconSizes[iconSizeIndex] * 0.22)
                height: width
                running: delegateRoot.previewLoading
                visible: running
                opacity: running ? 1 : 0
