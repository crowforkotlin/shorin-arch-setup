...
Singleton {
    ...
    Connections {
        ...
        function onNetworkStateUpdate(data) {
            ...
            console.debug("DMSNetworkService: Subscription update received, networks:", networksCount);
            updateState(data);
        }
    }
    ...
    function submitCredentials(token, secrets, save) {
        console.debug("DMSNetworkService submitCredentials: networkAvailable=" + networkAvailable + " apiVersion=" + DMSService.apiVersion);

        if (!networkAvailable || DMSService.apiVersion < 7) {
            console.warn("DMSNetworkService submitCredentials: Aborting - networkAvailable=" + networkAvailable + " apiVersion=" + DMSService.apiVersion);
            return;
        }
    ...
    function forgetWifiNetwork(ssid) {
        ...
        DMSService.sendRequest("network.wifi.forget", {
            ...
            if (response.error) {
                console.warn("DMSNetworkService: Failed to forget network:", response.error);
            } else {
    ...
    function toggleWifiRadio() {
        ...
        DMSService.sendRequest("network.wifi.toggle", null, response => {
            ...
            if (response.error) {
                console.warn("DMSNetworkService: Failed to toggle WiFi:", response.error);
    ...
    function setNetworkPreference(preference) {
        ...
        DMSService.sendRequest("network.preference.set", {
            ...
            if (response.error) {
                console.warn("DMSNetworkService: Failed to set network preference:", response.error);
            }
        });
    }
